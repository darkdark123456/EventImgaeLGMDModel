...
namespace HIOlAB_CXX_14 {
    ...
    template<typename T>
    static T ptp(const QVector<QVector<T>>& Array2D){
...
//        block_num < 25 ? actual_thread_num = 1 : void(0);
                ...
                for (int i = start_row; i < end_row; i++) {
                    for (int j = 0; j < columns; j++) {
                        try {
                             ...
                             if (Array2D[i][j] > threshold) { Array2D[i][j] = threshold; }
   ...
   /* template<typename T>
        ...
        for (int i = end_row; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                try {
                     ...
                     if (Array2D.at<T>(i, j) == threshold) Array2D.at<T>(i, j) = 0;
                }
                catch (const std::exception& outOfRangeException) {
    ...
    static  std::vector<T> booleanArrayIndexing(cv::Mat& InputArray, cv::Mat& mask) {
        ...
        if (block_num < 128) actual_thread_num = 1;
    ...
    template<typename T>
    static void reshape(Eigen::VectorXd& vector, int row, int col, Eigen::Matrix<T,Eigen::Dynamic,Eigen::Dynamic>& result) {
                ...
                for (int i = start_row; i < end_row; i++) {
                    ...
                    for (int j = 0; j < columns; j++) {
                        try {
                             ...
                             if (InputArray.at<T>(i, j) == value) {

                                 indexVecPair.first.push_back(i);
                                 indexVecPair.second.push_back(j);
                            }
                ...
                for (int i = start_row; i < end_row; i++) {
                    for (int j = 0; j < columns; j++) {
                        try {
                             ...
                             if (InputArray.at<T>(i, j) == value) mask.at<uchar>(i, j) = 1;
                        }
                        catch (const std::exception& outOfRangeException) {
    ...
    static void updateMatrix(cv::Mat& InputArray,cv::Mat& OutputArray,int count) {
        ...
        for (int i = 0; i < InputArray.cols; i++){
            ...
            try {
                ...
                if (!std::isnan(InputArray.at<T>(0, i))) {

                    OutputArray.at<T>(0, j) = InputArray.at<T>(0, i);
                    ++j;
                }
            }
            catch (const std::exception& outOfRangeException){
...
//void matToQImage(const cv::Mat& mat, QImage& qImage) {
//    // 确保图像类型为CV_8UC3
//    if (mat.channels() == 3) {
//        // 将cv::Mat转换为QImage
...
//        // 如果是灰度图像，则转换为QImage::Format_Grayscale8
