...
namespace OMDA_Algorithm {
	...
	static void calculate2DHeightFromSlope(cv::Mat& sx2d, cv::Mat& sy2d, cv::Mat& x2d, cv::Mat& y2d, cv::Mat& z2d_wfr) {
        ...
        Eigen::Map<Eigen::VectorXd> vec_gx5(mgx5_nan_value_vec.data(), mgx5_nan_value_vec.size());
        Eigen::Map<Eigen::VectorXd> vec_gy5(mgy5_nan_value_vec.data(), mgy5_nan_value_vec.size());
        Eigen::VectorXd matrix_g(vec_gx5.size() + vec_gy5.size());
        ...
        Eigen::VectorXd result = solver.solve(matrix_g);
