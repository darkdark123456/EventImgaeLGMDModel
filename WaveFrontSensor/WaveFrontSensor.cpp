...
bool WaveFrontSensor::event_widget_flag=false;
    ...
    , ui(new Ui::WaveFrontSensorClass())
{
    ...
    connect(parms_config_button_.data(), &QToolButton::clicked, this, &WaveFrontSensor::parmConfigButtonClicked);

    connect(select_data_tool_button_.data(), &QToolButton::clicked, this, &WaveFrontSensor::startButtonClicked);
...
void WaveFrontSensor::parmConfigButtonClicked(){
...
void WaveFrontSensor::startButtonClicked(){
...
//    }
    filename_ ="/home/jetson/桌面/test_qt/untitled3/WaveFrontSensor/data_example_21/ex21_res_int_pr_se.dat";
    infoText->clear();
    ...
    WaveFrontSensor::event_widget_flag=true;
}
...
void WaveFrontSensor::initDefaultDisplay() {
    ...
    palette.setBrush(QPalette::Window, QBrush(QPixmap(":/WaveFrontSensor/back/b1.png")));
    setPalette(palette);
    ...
    default:
     ...
     if (is_show) {
         ...
         { 
             ...
             //1             ok
             cv::Mat normal_mat;
             cv::normalize(wfr_wl, normal_mat, 0, 255,cv::NORM_MINMAX);
             normal_mat.convertTo(normal_mat, CV_8U); 
             cv::Mat heatmap;
             cv::applyColorMap(normal_mat, heatmap, cv::COLORMAP_VIRIDIS);
             cv::resize(heatmap, heatmap, cv::Size(), 8.0, 8.0, cv::INTER_LINEAR);

             QImage heat_imgae;

             //在jeson nano下非正常显示
             QPixmap hot_map_1("/home/jetson/桌面/test_qt/untitled3/WaveFrontSensor/h1.png");
             QPixmap hot_map_2("/home/jetson/桌面/test_qt/untitled3/WaveFrontSensor/h2.png");
             QPixmap hot_map_3("/home/jetson/桌面/test_qt/untitled3/WaveFrontSensor/h3.png");



             if (heatmap.channels() == 3) {
                 
                 heat_imgae = QImage(heatmap.data, heatmap.cols, heatmap.rows, heatmap.step, QImage::Format_RGB888);
             }
             else if (heatmap.channels() == 1) {
            
                 heat_imgae = QImage(heatmap.data, heatmap.cols, heatmap.rows, heatmap.step, QImage::Format_Grayscale8);
             }
             else {
                 heat_imgae = QImage();
             }
...
//             QGraphicsPixmapItem* pixmap_item = scene->addPixmap(QPixmap::fromImage(heat_imgae));
             QGraphicsPixmapItem* pixmap_item = scene->addPixmap(hot_map_1);
             ...
             wfr_coefs_wl.convertTo(abr_coefs_wl, CV_32F);

             std::vector<float> data2 = abr_coefs_wl.reshape(0, 1);
             QBarSet* set = new QBarSet("Wavefront Coefficients");
             ...
             chart->setTitle("Wavefront coefficients with Zernike models");
             ...
             axisY->setTitleText("Coefficients [λ]");
             ...
             QImage  heat_image_2;
             cv::Mat heat_map_2;
             cv::normalize(abr_wl, abr_wl, 0, 255, cv::NORM_MINMAX);
             abr_wl.convertTo(normal_mat, CV_8U);
             cv::applyColorMap(normal_mat,heat_map_2 , cv::COLORMAP_VIRIDIS);
             cv::resize(heat_map_2, heat_map_2, cv::Size(), 8.0, 8.0, cv::INTER_LINEAR);

           //  cv::imwrite("/home/jetson/桌面/test_qt/untitled3/WaveFrontSensor/h22.png", heat_map_2);

             QImage heat_image_3;
             cv::Mat heat_map_3;
             cv::normalize(abr_models_wl, abr_models_wl, 0, 255, cv::NORM_MINMAX);
             abr_models_wl.convertTo(normal_mat, CV_8U);
             cv::applyColorMap(normal_mat, heat_map_3, cv::COLORMAP_VIRIDIS);
             cv::resize(heat_map_3, heat_map_3, cv::Size(), 8.0, 8.0, cv::INTER_LINEAR);

           // cv::imwrite("/home/jetson/桌面/test_qt/untitled3/WaveFrontSensor/h33.png", heat_map_3);


             if (heat_map_2.channels() == 3) {

                 heat_image_2 = QImage(heat_map_2.data, heat_map_2.cols, heat_map_2.rows, heat_map_2.step, QImage::Format_RGB888);
             }
             else if (heat_map_2.channels() == 1) {

                 heat_image_2 = QImage(heat_map_2.data, heat_map_2.cols, heat_map_2.rows, heat_map_2.step, QImage::Format_Grayscale8);
             }
             else
             {
                 heat_image_2 = QImage();
             ...
             if (heat_map_3.channels() == 3) {

                 heat_image_3 = QImage(heat_map_3.data, heat_map_3.cols, heat_map_3.rows, heat_map_3.step, QImage::Format_RGB888);
             }
             else if (heat_map_3.channels() == 1) {

                 heat_image_3 = QImage(heat_map_3.data, heat_map_3.cols, heat_map_3.rows, heat_map_3.step, QImage::Format_Grayscale8);
             }
             else
             {
                 heat_image_3 = QImage();
             ...
             //QGraphicsPixmapItem* pixmap_item_2 = scene->addPixmap(QPixmap::fromImage(heat_image_2));
             QGraphicsPixmapItem* pixmap_item_2 = scene->addPixmap(hot_map_2);
             pixmap_item_2->setPos(heat_imgae.width(), 0);


            // QGraphicsPixmapItem* pixmap_item_3 = scene->addPixmap(QPixmap::fromImage(heat_image_3));
             QGraphicsPixmapItem* pixmap_item_3 = scene->addPixmap(hot_map_3);

             pixmap_item_3->setPos(heat_imgae.width()+heat_image_2.width(), 0);


             scene->setSceneRect(scene->itemsBoundingRect());
             QGraphicsView* view = new QGraphicsView(scene);
             view->setWindowTitle("wavefont with legendre");
             view->setFixedSize(QSize(450, 300));
